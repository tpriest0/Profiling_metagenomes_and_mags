...
The best way to do this is to first run the workflow on metagenome samples (and adjust the config.yaml file so that the WORKING_DIR ends with something like 'MG_analysis') and then run on metatranscriptome samples (adjusting the WORKING_DIR to 'MT_analysis').
